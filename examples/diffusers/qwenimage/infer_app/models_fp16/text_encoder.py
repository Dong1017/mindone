...
from mindone.transformers.modeling_layers import GradientCheckpointingLayer
...
class Qwen2_5_VisionRotaryEmbedding(nn.Cell):
    def __init__(self, dim: int, theta: float = 10000.0) -> None:
        super().__init__()
        inv_freq = 1.0 / (theta ** (mint.arange(0, dim, 2, dtype=ms.float) / dim))
...
def eager_attention_forward(
    ...
    **kwargs,
    ...
    key_states = repeat_kv(key.to(ms.float32), module.num_key_value_groups)
...
class Qwen2_5_VLVisionAttention(nn.Cell):
...
class Qwen2_5_VLPreTrainedModel(MSPreTrainedModel):
...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        self.blocks = nn.CellList([Qwen2_5_VLVisionBlock(config) for _ in range(config.depth)])
...
class Qwen2_5_VLAttention(nn.Cell):
    """
    ...
    def construct(
        ...
        position_embeddings: Optional[ms.Tensor] = None,  # necessary, but kept here for BC
    ...
    def get_rope_index(
        ...
        Explanation:
            ...
            Examples:
                ...
                tokens_per_second: This is a crucial parameter. It dictates how many "time-steps" or "temporal tokens"
                    are conceptually packed into a one-second interval of the video. In this case, we have 25 tokens per second.
        ...
        if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                # TODO: ops.argwhere -> mint.argwhere
                ...
                for _ in range(image_nums + video_nums):
                    ...
                    # normalize type
                    second_per_grid_t = ms.tensor(second_per_grid_t, dtype=range_tensor.dtype)
        ...
        if pixel_values is not None:
            ...
            if input_ids is None:
                image_mask = inputs_embeds == self.get_input_embeddings()(
                    ms.tensor(self.config.image_token_id, dtype=ms.long)
                )
        ...
        if pixel_values_videos is not None:
            ...
            if input_ids is None:
                video_mask = inputs_embeds == self.get_input_embeddings()(
                    ms.tensor(self.config.video_token_id, dtype=ms.long)
                )
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def _get_image_nums_and_video_nums(
        ...
        if inputs_embeds is not None:
            vision_start_mask = (
                inputs_embeds == self.get_input_embeddings()(ms.tensor(vision_start_token_id, dtype=ms.long))
            )[..., 0]
            image_mask = (inputs_embeds == self.get_input_embeddings()(ms.tensor(image_token_id, dtype=ms.long)))[
                ..., 0
            ]
            video_mask = (inputs_embeds == self.get_input_embeddings()(ms.tensor(video_token_id, dtype=ms.long)))[
                ..., 0
            ]
        else:
